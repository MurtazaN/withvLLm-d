"""
Blue Lantern Benchmark Harness
...
#   - Host dev:    defaults to blue-lantern/benchmark/results/
...
async def run_benchmark(max_alerts: int = 30) -> dict:
    ...
    Alerts are processed concurrently up to `BLUE_LANTERN_CONCURRENCY` (default
    ...
    print(f"\n{'='*70}")
    print(f"Blue Lantern Benchmark — {n_total} alerts (concurrency={concurrency})")
    print(f"{'='*70}\n")
...
if __name__ == "__main__":
    ...
    # BLUE_LANTERN_LOG_LEVEL=INFO for CI / production where the JSON stream
    # is the point of running the harness.
    os.environ.setdefault("BLUE_LANTERN_LOG_LEVEL", "WARNING")
    setup_logging()
    setup_tracing()
