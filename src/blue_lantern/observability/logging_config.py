...
Environment knobs:
- ``BLUE_LANTERN_LOG_LEVEL``  — ``DEBUG``/``INFO``/``WARNING``/``ERROR``;
                            defaults to ``INFO``.
- ``BLUE_LANTERN_LOG_FILE``   — when set, JSON appends to this path
                            instead of stderr.
"""
...
def setup_logging() -> None:
    ...
    level_name = os.environ.get("BLUE_LANTERN_LOG_LEVEL", "INFO").upper()
    ...
    log_file = os.environ.get("BLUE_LANTERN_LOG_FILE")
    if log_file:
